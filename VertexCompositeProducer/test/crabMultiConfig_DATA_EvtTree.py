...
config.JobType.psetName = 'pPbSkimAndTree2016_MinimumBiasCounting_cfg.py'
config.JobType.inputFiles = ['lumiData.csv', 'lumiDataMB.csv', 'lumiDataHM.csv']
...
config.Site.whitelist = ['T1_US_*','T2_US_*','T1_FR_*','T2_FR_*','T2_CH_CERN','T2_BE_*']
...
for key, val in dataMap.items():
    config.General.requestName = 'EventTree_'+key+'_PARun2016C_20200619'
    config.Data.inputDataset = val["PD"]
    config.Data.unitsPerJob = val["Units"]
    config.JobType.maxMemoryMB = val["Memory"]
    config.JobType.maxJobRuntimeMin = val["RunTime"]
    config.Data.outputDatasetTag = config.General.requestName
    config.Data.outLFNDirBase = '/store/group/phys_heavyions/anstahll/RiceHIN/pPb2016/TREE/%s' % (config.General.requestName)
