...
// Class:      D0FitterNew
// 
/**\class D0FitterNew D0FitterNew.cc VertexCompositeAnalysis/VertexCompositeProducer/src/D0FitterNew.cc
...
const float kaonMassD0New = 0.493677;
const float kaonMassD0NewSquared = kaonMassD0New*kaonMassD0New;
const float d0MassD0New = 1.86484;
float piMassD0New_sigma = 3.5E-7f;
float kaonMassD0New_sigma = 1.6E-5f;
...
void D0FitterNew::fitAll(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
...
//      TransientTrack tmpTk( *tmpRef, &(*bFieldHandle), globTkGeomHandle );
  ...
  float posCandMass[2] = {piMassD0New, kaonMassD0New};
  float negCandMass[2] = {kaonMassD0New, piMassD0New};
  float posCandMass_sigma[2] = {piMassD0New_sigma, kaonMassD0New_sigma};
  float negCandMass_sigma[2] = {kaonMassD0New_sigma, piMassD0New_sigma};
...
//          || std::abs(cxPt.z()) > 300.) continue;
      ...
      double totalE1 = sqrt( posTSCP.momentum().mag2() + kaonMassD0NewSquared ) +
                      sqrt( negTSCP.momentum().mag2() + piMassD0NewSquared );
      double totalE1Sq = totalE1*totalE1;

      double totalE2 = sqrt( posTSCP.momentum().mag2() + piMassD0NewSquared ) +
                      sqrt( negTSCP.momentum().mag2() + kaonMassD0NewSquared );
      ...
      d0Particles.push_back(pFactory.particle(*posTransTkPtr,posCandMass[0],chi,ndf,posCandMass_sigma[0]));
      d0Particles.push_back(pFactory.particle(*negTransTkPtr,negCandMass[0],chi,ndf,negCandMass_sigma[0]));
      ...
      for(int i=0;i<2;i++)
      {
        ...
        addp4.set( *theD0 );
        if( theD0->mass() < d0MassD0New + d0MassCut &&
            theD0->mass() > d0MassD0New - d0MassCut ) //&&
	   // theD0->pt() > dPtCut ) {
        {
...
const reco::VertexCompositeCandidateCollection& D0FitterNew::getD0() const {
  return theD0s;
}

const std::vector<float>& D0FitterNew::getMVAVals() const {
  return mvaVals_;
}

/*
auto_ptr<edm::ValueMap<float> > D0FitterNew::getMVAMap() const {
  return mvaValValueMap;
}
*/

void D0FitterNew::resetAll() {
    theD0s.clear();
    mvaVals_.clear();
}
