...
void v0polinjet(const TString& inputList = "filelist_v0.txt")
{
  ...
  std::unique_ptr<TH2D> hJet4KsRecoAll_pol(new TH2D("hJet4KsRecoAll_pol", "All channels, Ks p3;cos(#theta*);mass (GeV);",
          400, -1, 1, 80, 0.44, 0.55));

  std::unique_ptr<TH2D> hJet8KsRecoAll_pol(new TH2D("hJet8KsRecoAll_pol", "All channels, Ks p3;cos(#theta*);mass (GeV);",
          400, -1, 1, 80, 0.44, 0.55));
  ...
  std::unique_ptr<TH2D> hJet4LamRecoAll_pol(new TH2D("hJet4LamRecoAll_pol", "All channels, Lam p3;cos(#theta*);mass (GeV);",
          400, -1, 1, 80, 1.08, 1.16));

  std::unique_ptr<TH2D> hJet8LamRecoAll_pol(new TH2D("hJet8LamRecoAll_pol", "All channels, Lam p3;cos(#theta*);mass (GeV);",
          400, -1, 1, 80, 1.08, 1.16));
  ...
  TH2D* hJet4KsRecoMult_pol[100];
  TH2D* hJet8KsRecoMult_pol[100];
  TH2D* hJet4LamRecoMult_mass[100];
  TH2D* hJet8LamRecoMult_mass[100];
  TH2D* hJet4LamRecoMult_pol[100];
  TH2D* hJet8LamRecoMult_pol[100];
  ...
  for(int i=0;i<7;i++)
  {
    ...
    hJet4KsRecoMult_pol[i] = new TH2D(Form("hJet4KsRecoMult_pol_%d",i), "All channels, Ks p3;cos(#theta*);mass (GeV);",
          400, -1, 1, 80, 0.44, 0.55);
    hJet8KsRecoMult_pol[i] = new TH2D(Form("hJet8KsRecoMult_pol_%d",i), "All channels, Ks p3;cos(#theta*);mass (GeV);",
          400, -1, 1, 80, 0.44, 0.55);
    ...
    hJet4LamRecoMult_pol[i] = new TH2D(Form("hJet4LamRecoMult_pol_%d",i), "All channels, Lam p3;cos(#theta*);mass (GeV);",
          400, -1, 1, 80, 1.08, 1.16);
    hJet8LamRecoMult_pol[i] = new TH2D(Form("hJet8LamRecoMult_pol_%d",i), "All channels, Lam p3;cos(#theta*);mass (GeV);",
          400, -1, 1, 80, 1.08, 1.16);
  ...
  for (Long64_t ientry=0; ientry<nentries; ientry++) {
    ...
    if(jetpt>500)
    {
      ...
      for (size_t ireco=0; ireco<recosize_ks; ireco++) {
         ...
         TLorentzVector pDau_p4;
         if(p_ks.cand_charge()[dauIdx.at(0)]>0) pDau_p4.SetPtEtaPhiM(p_ks.cand_pT()[dauIdx.at(0)],p_ks.cand_eta()[dauIdx.at(0)],p_ks.cand_phi()[dauIdx.at(0)],p_ks.cand_mass()[dauIdx.at(0)]);
         else pDau_p4.SetPtEtaPhiM(p_ks.cand_pT()[dauIdx.at(1)],p_ks.cand_eta()[dauIdx.at(1)],p_ks.cand_phi()[dauIdx.at(1)],p_ks.cand_mass()[dauIdx.at(1)]);

         TLorentzVector ks_p4;
         ks_p4.SetPtEtaPhiM(p_ks.cand_pT()[ireco],p_ks.cand_eta()[ireco],p_ks.cand_phi()[ireco],p_ks.cand_mass()[ireco]);
         TVector3 ks = ks_p4.Vect();

         pDau_p4.Boost(-ks_p4.BoostVector());
         TVector3 pDau = pDau_p4.Vect();

         TVector3 norm = ks.Cross(jet);
         double costheta_pol = pDau.Dot(norm)/pDau.Mag()/norm.Mag();
         ...
         if(ks.DeltaR(jet)<0.4) 
         {
           ...
           hJet4KsRecoAll_pol->Fill(costheta_pol, p_ks.cand_mass()[ireco]);
           if(p_ks.cand_pT()[ireco]>3.0) hJet4KsRecoMult_pol[multBin]->Fill(costheta_pol, p_ks.cand_mass()[ireco]);
         }
         if(ks.DeltaR(jet)<0.8)
         {
           ...
           hJet8KsRecoAll_pol->Fill(costheta_pol, p_ks.cand_mass()[ireco]);
           if(p_ks.cand_pT()[ireco]>3.0) hJet8KsRecoMult_pol[multBin]->Fill(costheta_pol, p_ks.cand_mass()[ireco]);
         }
      }
      ...
      for (size_t ireco=0; ireco<recosize_lam; ireco++) {
         ...
         TLorentzVector pDau_p4;
         if(p_lam.cand_charge()[dauIdx.at(0)]>0) pDau_p4.SetPtEtaPhiM(p_lam.cand_pT()[dauIdx.at(0)],p_lam.cand_eta()[dauIdx.at(0)],p_lam.cand_phi()[dauIdx.at(0)],p_lam.cand_mass()[dauIdx.at(0)]);
         else pDau_p4.SetPtEtaPhiM(p_lam.cand_pT()[dauIdx.at(1)],p_lam.cand_eta()[dauIdx.at(1)],p_lam.cand_phi()[dauIdx.at(1)],p_lam.cand_mass()[dauIdx.at(1)]);
...
*/
         ...
         TLorentzVector lam_p4;
         lam_p4.SetPtEtaPhiM(p_lam.cand_pT()[ireco],p_lam.cand_eta()[ireco],p_lam.cand_phi()[ireco],p_lam.cand_mass()[ireco]);
         TVector3 lam = lam_p4.Vect();

         pDau_p4.Boost(-lam_p4.BoostVector());
         TVector3 pDau = pDau_p4.Vect();

         TVector3 norm = lam.Cross(jet);
         double costheta_pol = pDau.Dot(norm)/pDau.Mag()/norm.Mag();      
         ...
         if(lam.DeltaR(jet)<0.4)
         {
           ...
           hJet4LamRecoAll_pol->Fill(costheta_pol, p_lam.cand_mass()[ireco]);
           if(p_lam.cand_pT()[ireco]>3.0) hJet4LamRecoMult_pol[multBin]->Fill(costheta_pol, p_lam.cand_mass()[ireco]);
         }
         if(lam.DeltaR(jet)<0.8)
         {
           ...
           hJet8LamRecoAll_pol->Fill(costheta_pol, p_lam.cand_mass()[ireco]);
           if(p_lam.cand_pT()[ireco]>3.0) hJet8LamRecoMult_pol[multBin]->Fill(costheta_pol, p_lam.cand_mass()[ireco]);
         }
      }
...
//         if(sqrt(pow(p_antixi.trk_xyDCASignificance().at(gtrkIdx1),2)+pow(p_antixi.trk_zDCASignificance().at(gtrkIdx1),2))>45) continue;
  ...
  TFile* fout = new TFile(Form("outputs/v0polinjet_output_%s.root",inputList.Data()),"recreate");
  ...
  hJet4LamRecoAll_pol->Write();
  hJet8LamRecoAll_pol->Write();
  ...
  for(int i=0;i<7;i++)
  {
    ...
    hJet4KsRecoMult_pol[i]->Write();
    hJet8KsRecoMult_pol[i]->Write();
    hJet4LamRecoMult_mass[i]->Write();
    hJet8LamRecoMult_mass[i]->Write();
    hJet4LamRecoMult_pol[i]->Write();
    hJet8LamRecoMult_pol[i]->Write();
