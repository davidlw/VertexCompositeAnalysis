...
void v0()
{
  ...
  for (Long64_t ientry=0; ientry<nentries; ientry++) {
    ...
    for (size_t ireco=0; ireco<recosize_ks; ireco++) {
       ...
       if(p_ks.cand_decayLength3D()[ireco]/p_ks.cand_decayLengthError3D()[ireco]<3) continue;
    ...
    for (size_t ireco=0; ireco<recosize_lam; ireco++) {
       ...
       if(p_lam.cand_decayLength3D()[ireco]/p_lam.cand_decayLengthError3D()[ireco]<3) continue;
    ...
    for (size_t ireco=0; ireco<recosize_xi; ireco++) {
       ...
       if(cos(p_xi.cand_angle3D()[ireco])<0.999) continue;

       auto dauIdx = dauIdxEvt_xi.at(ireco);
       if(p_xi.cand_decayLength3D()[dauIdx[1]]/p_xi.cand_decayLengthError3D()[dauIdx[1]]<3) continue;

       auto trkIdx0 = p_xi.cand_trkIdx().at(dauIdx.at(0));
       if(p_xi.trk_xyDCASignificance().at(trkIdx0)<3) continue;
       if(p_xi.trk_zDCASignificance().at(trkIdx0)<3) continue;
    ...
    for (size_t ireco=0; ireco<recosize_om; ireco++) {
       ...
       if(cos(p_om.cand_angle3D()[ireco])<0.999) continue;

       auto dauIdx = dauIdxEvt_om.at(ireco);
       if(p_om.cand_decayLength3D()[dauIdx[1]]/p_om.cand_decayLengthError3D()[dauIdx[1]]<3) continue;

       auto trkIdx0 = p_om.cand_trkIdx().at(dauIdx.at(0));
       if(p_om.trk_xyDCASignificance().at(trkIdx0)<3) continue;
       if(p_om.trk_zDCASignificance().at(trkIdx0)<3) continue;
  ...
  TFile* fout = new TFile("v0_output.root","recreate");
